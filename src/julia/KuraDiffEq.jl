...
load_data = :(begin
    ...
    for row in results
        ...
        global const coupling_constant = row.coupling_constant
    end
...
tstartup = @CPUelapsed begin
	using LightGraphs: SimpleGraph, incidence_matrix
...
# find a reference experiment or annouce it doesn't exist
# refid = select id from experiments where system={systemid} and tend=$tend and atol=1e-12 and rtol=1e-10
results =DBInterface.execute(db,string(
